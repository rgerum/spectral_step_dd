...
def get_train(net, trainloader, testloader, device, optimizer, criterion, output_folder):
    ...
    def func_test(epoch):
        ...
        net.eval()
        with torch.no_grad():
            acc_val, loss_val = get_accuracy(net, criterion, testloader, device)
            acc_train, loss_train = get_accuracy(net, criterion, trainloader, device)
        ...
        return dict(acc_val=acc_val, loss_val=loss_val, acc_train=acc_train, loss_train=loss_train,
                    time=time.time(), epoch=epoch)

    return func_train, func_test


def get_accuracy(net, criterion, testloader, device):
    test_loss = 0
    correct = 0
    total = 0
    with torch.no_grad():
